"""Quick pull July 6 + compute live profit"""
...
d = datetime(2026, 7, 6)
after = d.strftime('%Y-%m-%dT07:00:00Z')
    ...
    for region, mids in regions:
        ...
        while True:
            ...
            for o in orders:
                ...
                if ct:
                    try:
                        ld = (datetime.fromisoformat(str(ct).replace('Z', '+00:00')) - timedelta(hours=7)).date()
                    except:
                        ld = None
                    if ld != d.date(): continue
...
conn.close()
print(f'Orders loaded: {len(all_items)} items')

# Compute live profit (run sync version)
...
for it in all_items:
    ...
    if result and 'error' not in result:
        ...
        if f != 'OK': flagged += 1
        est = '⚠️ ' if f != 'OK' else '✓ '
        print(f'{est} {it["store"][:6]:6} {it["mkt"]:3} {it["asin"]:14} {it["qty"]}x ${it["price"]:.2f} → rev ${result["revenue"]:.2f} profit ${result["profit"]:.2f} [{f}]')

if flagged:
    print(f'\n⚠️ {flagged} items with missing costs — need catalog updates')
...
for row in s['summary']:
    print(f'{row["store"]:10} {row["marketplace"]:>4} {row["orders"]:>5} orders {row["units"]:>5} units rev ${row["revenue"]:>10,.2f} profit ${row["profit"]:>10,.2f} ({row["margin"]}%)')
t = s['totals']
print(f'{"TOTAL":10}     {t["orders"]:>5} orders {t["units"]:>5} units rev ${t["revenue"]:>10,.2f} profit ${t["profit"]:>10,.2f} ({t["margin"]}%)')
