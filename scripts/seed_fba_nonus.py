"""Seed fba_fees for non-US markets"""
...
# Get non-US ASINs NOT already in fba_fees
cur.execute("""SELECT DISTINCT oa.marketplace, oi.asin, MAX(oi.unit_price) FROM orders_active oa
    ...
    WHERE oa.purchase_date>'2026-06-01' AND oa.marketplace != 'US'
    AND oi.unit_price > 0
...
conn.close()
print(f'Non-US ASINs to fetch: {len(asins)}')

if not asins:
    print('All fetched already!')
    exit(0)
...
results = []; fetched = 0; errors = 0
lock = threading.Lock(); sem = threading.Semaphore(3)

def fetch_one(mkt, asin, price):
    global fetched, errors
    sem.acquire()
    try:
        region = REGION_MAP.get(mkt, 'NA'); tk = tokens.get(region)
        if not tk: errors += 1; sem.release(); return
        host = HOSTS[region]; mid = MID_MAP.get(mkt, 'ATVPDKIKX0DER')
        body = {'FeesEstimateRequest': {'MarketplaceId': mid, 'IsAmazonFulfilled': True, 'PriceToEstimateFees': {'ListingPrice': {'CurrencyCode': 'USD', 'Amount': price}}, 'Identifier': asin}}
        r2 = httpx.post(f'https://{host}/products/fees/v0/items/{asin}/feesEstimate', headers={'x-amz-access-token': tk, 'Content-Type': 'application/json'}, json=body, timeout=15)
        if r2.status_code == 200:
            fe = r2.json().get('payload', r2.json()).get('FeesEstimateResult', {}).get('FeesEstimate', {})
            fba = 0
            for f in fe.get('FeeDetailList', []):
                if f.get('FeeType') in ('FBAFees',): fba = float(f.get('FinalFee', {}).get('Amount', 0))
            if fba > 0:
                results.append((asin, mkt, fba))
                fetched += 1; print(f'{fetched} {mkt} {asin}: \${fba}')
        else: errors += 1
    except: errors += 1
    finally: sem.release()
...
for mkt, asin, price in asins:
    t = threading.Thread(target=fetch_one, args=(mkt, asin, price)); t.start(); threads.append(t); time.sleep(0.03)
for t in threads: t.join()

conn = psycopg2.connect(dsn, connect_timeout=10); cur = conn.cursor()
